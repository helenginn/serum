...
public:
	...
	double aveVectorCompare(Strain *str);
	void sort();
	...
	void findPosition();
	void findAvePosition();
	vec3 aveDirection();
	void calculateCloud();
	
	void addChild(Serum *s)
	{
	...
	void setChequered(bool ch)
	{
		_chequered = ch;
	}
	
	void recolourObject(SlipObject *obj);
	void incrementUses();
...
private:
	...
	/* sera that have been tested against strain */
	std::vector<Serum *> _sera;

	/* sera which have been generated by infection by strain */
	std::vector<Serum *> _children;
